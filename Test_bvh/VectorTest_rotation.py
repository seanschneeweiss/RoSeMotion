...
x_prev = np.array([0.981514215469, -0.161985740066, -0.101933889091])
y_prev = np.array([0.17750826478, 0.969606161118, 0.168388620019])
z_prev = np.array([0.0715591683984, -0.183369934559, 0.98043602705])
x_next = np.array([0.983236432076, -0.165484800935, -0.0765564069152])
y_next = np.array([0.171309798956, 0.982199370861, 0.0770538300276])
z_next = np.array([0.0624424181879, -0.0888769999146, 0.994083404541])


rotm_prev = np.transpose(np.array([x_prev, y_prev, z_prev]))
rotm_next = np.transpose(np.array([x_next, y_next, z_next]))

print(rotm_prev)
print(rotm_next)

print(np.transpose(quat2mat(quat_between_rot(rotm_prev, rotm_next))))
print(np.matmul(rotm_next, np.transpose(rotm_prev)))
